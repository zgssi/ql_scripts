...
async function get_booking_msg(){
    return new Promise((reso) => {
        $.post(postUrl("showSecondFloorAppointmentInfo", {"source":"appointment"}), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} get_booking_msg API请求失败，请检查网路重试`)
                } else {
                    data = JSON.parse(data);
                    if (data.code === "0" && data.data.bizCode === "0"){
                        $.taskList = data.data.result.activityRewards.encryptAssignmentNames;
...
async function do_booking(activityId, ProjectId, AssignmentName){
    return new Promise((reso) => {
        $.post(postUrl("superBrandTaskLotteryByName", {"source":"appointment","activityId":activityId,"encryptProjectId":ProjectId,"encryptAssignmentName":AssignmentName}), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} get_booking_msg API请求失败，请检查网路重试`)
                    ...
                    if (data.code === "0" && data.data.success === true){
                        if (data.data.result.userAwardInfo){
                            console.log(`${data.data.bizMsg}获得${data.data.result.userAwardInfo.beanNum}京豆\n`)
                        }else{
