...
日常任务，助力，游戏
第一个账号助力作者 其他依次助力CK1
默认不做加购任务，如需要，请设置变量erport ADD_CART='true'
...
$.hotFlag=false;
$.outFlag=false;
let shareUuidArr=['l98VBIYyK9fWGV9YC3SoIGBUh0gimxufjFRMgNwy0cYiU+N6ZQy0LI2KZrs00RAY','+dhJsKN+TNn9G9RfE3geesjNhNaYFy2HteErE6izlhTf9nrGY7gBkCdGU4C6z/xD'];
let n=0;
n=Math.floor(Math.random()*shareUuidArr.length);
let shareUuid=shareUuidArr[n]||'';
!(async()=>{
	console.log('活动快捷入口：11:/！EAlpe6sVE3！ ，【綡﹎Dōδδng】参与头文字J，集能量兑换京豆');
    if (!cookiesArr[0]) {
        ...
        for (let i = 0; i < $.taskLists.length; i++){
            ...
            switch($.missionType){
                ...
                case 'collectShop':
                    ...
                    break;
                case 'addCart':
                    if (process.env.ADD_CART && process.env.ADD_CART === 'true'){
                    for(let i=0;i<3;i++){
                    ...
                    } else {console.log(`默认不加购,请设置变量export ADD_CART='true'做加购任务`)}
                    break;
                default:
                    await takePostRequest('doTask');
                    await $.wait(1000);  
