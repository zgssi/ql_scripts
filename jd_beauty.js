...
cron 20 7,12,19 * * * jd_beautyStudy.js, tag=美丽研究院, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
 */
const $ = new Env('美丽研究院');
        ...
        case "produce_position_info_v2":
          ...
          if (vo.data && vo.data.material_name !== '') {
            ...
            if (new Date().getTime() > vo.data.procedure.end_at) {
              ...
              await $.wait(5000);
              client.send(`{"msg":{"type":"action","args":{"position":"${oc(() => vo.data.position)}"},"action":"to_employee_v2"}}`)
              await $.wait(5000);
